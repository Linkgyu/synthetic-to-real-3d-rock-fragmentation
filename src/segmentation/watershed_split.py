"""Height-marker splitting for large surface clusters.

This module implements a lightweight marker-based split inspired by watershed
segmentation on a height field. It avoids an additional dependency on scikit-
image while giving us a practical way to break merged graph components.
"""
...
def _height_markers_xy(points: np.ndarray, grid_resolution_m: float, min_peak_distance_m: float, peak_prominence_m: float) -> np.ndarray:
    xy = points[:, :2]
    z = points[:, 2]
...
def split_large_clusters_by_height_markers(
    points_xyz: np.ndarray,
    labels: np.ndarray,
    max_cluster_points: int = 650,
    grid_resolution_m: float = 0.035,
    min_peak_distance_m: float = 0.14,
    peak_prominence_m: float = 0.025,
    min_child_points: int = 40,
    max_markers_per_cluster: int = 8,
) -> np.ndarray:
    """Split oversized clusters using local maxima in an XY height field.

    The method is intentionally conservative: clusters below max_cluster_points
    are left untouched, and splits that produce tiny children are rejected.
    """
    ...
    for label in sorted(set(labels)):
        ...
        _, child = marker_tree.query(cluster[:, :2], k=1)
        ...
        for child_id in range(len(markers)):
            ...
            if len(child_idx) >= min_child_points:
                output[child_idx] = next_label
                next_label += 1

    return _relabel_nonnegative(output)
